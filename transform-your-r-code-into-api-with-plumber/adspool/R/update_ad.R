...
#' @return dataframe
#' @export
update_ad <- function(ad_id, ad_name = NULL, ad_cat = NULL, ad_subcat = NULL, ad_customer = NULL, ad_click_count = NULL, ad_click_rate = NULL) {
    ...
    if (is.null(ad_customer)) ad_customer <- ad_to_update$customer_id
    ...
    updated.ads[updated.ads$id == ad_id,
               c("name", "category", "subcategory", "customer_id", "click_count", "click_rate")] <-
        c(ad_name, ad_cat, ad_subcat, ad_customer, ad_click_count, ad_click_rate)
