...
testthat::with_reporter("Progress", {
  ...
  testthat::test_that("GET client ads is ok", {
    res <- adspool$serve(make_req("GET", "/client/1"), plumber:::PlumberResponse$new())
    client1_ads <- jsonlite::fromJSON(res$body)
    testthat::expect_equal(unique(client1_ads$client_id), 1)
  })
