...
#* Return all the ads of a given customer id. Dynamic route.
#* @tag Rendering output.
#* @param ad_customer The id of the customer of the ads to get.
#* @get /customer/<ad_customer:int>
get_customers_ads <- function(ad_customer = NULL) {
    subset_ads(ad_customer = ad_customer)
}

# Image serializers @png and @jpeg cause the graphical output of an endpoint to be written to
# a file and then returned to the customer using jpeg() or png(). The annotation accepts parameters in this case.
...
#* Get invoice pdf for a given customer id.
#* @tag Rendering output.
#* @param id id of customer
#* @serializer contentType list(type="application/pdf")
#* @get /openinvoices/customer/<id:int>
get_openinvoices <- function(id, res){
    ...
    suppressMessages(
        rmarkdown::render(
            ...
            params = list(customer_id = id,
                          invoice_detail = invoice_detail,
                          invoice_summary = invoice_summary)
        )    
    )
    
    # This line tells the customer to handle the content as an attachment
...
#* @param ad_customer id of the customer owner of the ad
#* @param ad_subcat The subgategories of the ads to get.
#* @param ad_cat The categories of the ads to get.
#* @param ad_name The names of the ads to get.
#* @post /ads
post_create_ad <- function(ad_name, ad_cat = NULL, ad_subcat = NULL, ad_customer, ad_click_rate, res) {
...
post_create_ad_postbody <- function(req) {
    ...
    add_ad(newad = body$ad_name,
           ...
           newad_customer = body$ad_customer,
...
if (FALSE) {
    resp <- httr::PUT(
        ...
        body = list(ad_name = "Test1",                              # body
                    ...
                    ad_customer = 1,
                    ad_click_rate = 0.1),
        query = list(ad_customer = 5, ad_click_rate = 0.05))          # parameters to put_modify_ad, in the query
    httr::content(resp) %>% as.data.frame
}
...
filter_errorhandler <- function(req, res){
    if (req$REQUEST_METHOD == "POST" && grepl("^/ad$", req$PATH_INFO)) {
        req_params <- c("ad_name", "ad_customer", "ad_click_rate") %>% 
